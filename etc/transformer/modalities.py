...
class SymbolModality(modality.Modality):
  ...
  def get_w2v_embedding_matrix(self):
      ...
      for i in range(self._vocab_size):
          tok = self._model_hparams.problem.vocabulary.decode_one(i)
          vocab_tokens.append(tok)
      print('Done')
      vocab_emb = glove.embedding(vocab_tokens, maxlen=1)
      i = 0
      emb_dict = {}
      emb_dict['f'] = (np.random.rand(150)-.5)*2*np.sqrt(3)
      emb_dict['v'] = (np.random.rand(150)-.5)*2*np.sqrt(3)
      emb_dict['c'] = (np.random.rand(150)-.5)*2*np.sqrt(3)
      for token,emb in zip(vocab_tokens,vocab_emb):
          if len(token)>=4 and token[0]=='<' and token[3]=='>' and token[2].isdigit():
              if token[2] in emb_dict:
                  ...
                  emb_dict[token[2]] = (np.random.rand(150)-.5)*2*np.sqrt(3)
                  right = emb_dict[token[2]]
              ...
              if token[2:4] in emb_dict:
                  ...
                  emb_dict[token[2:4]] = (np.random.rand(150)-.5)*2*np.sqrt(3)
                  right = emb_dict[token[2:4]]
              re = np.concatenate((emb_dict[token[1]],right))
              assert re.shape==(300,)
              vocab_emb[i]=re 
