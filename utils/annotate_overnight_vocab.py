...
from redenote import get_fields
import re
def _chech_sketch(S):
    return True
...
def denotate(s='test'):
    ...
    question_file = os.path.join(path, '%s_vocab.qu'%s)
    #original lon file with qualified lon
    lon_file0 = os.path.join(path, '%s_%s0_vocab.lon'%(sub,s))
    ...
    lo_file = os.path.join(path, '%s_vocab.lon'%s)
